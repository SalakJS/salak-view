...
module.exports = (options = {}, app) => {
  ...
  return {
    controller: {
      async renderView (relPath, locals, module) {
        if (!module) {
          module = this.module
        }

        const html = await this.ctx.renderView(relPath, locals, module)
        return html
      },
      async render (relPath, locals, module) {
        ...
        await this.ctx.render(relPath, locals, module)
      }
    },
    context: {
      async renderView (relPath, locals = {}, module = app.root) {
      ...
      async render (relPath, locals, module = app.root) {
        this.type = 'text/html'
        this.body = await this.renderView(relPath, locals, module)
      }
    }
  }
}
